      ...
      <div className="p-4">
        <div className="flex items-start gap-4">
          ...
          <div className="w-12 h-12 rounded-lg bg-surface-700/50 flex-shrink-0 overflow-hidden">
            {product.image_url ? (
              <img src={`/api/products/image-proxy?url=${encodeURIComponent(product.image_url)}`} alt="" className="w-full h-full object-cover" />
            ) : (
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-1 flex-wrap">
              <h3 className="text-sm font-semibold text-white truncate mr-1">{product.name}</h3>
              
              {/* Badge de Actividad del Bot (Polling) */}
              {(product.is_active && ['monitoring', 'in_stock', 'purchasing', 'error'].includes(product.status)) ? (
                <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[11px] font-medium border bg-amber-500/15 text-amber-400 border-amber-500/20" title="Proceso en segundo plano activo">
                  <Eye size={10} /> Monitorizando
                </span>
              ) : (
                <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[11px] font-medium border bg-surface-500/15 text-surface-200/50 border-surface-500/20" title="Bot detenido para este artículo">
                  <Pause size={10} /> Pausado
                </span>
              )}

              {/* Badge de Estado Específico (Resultado) */}
              {product.status !== 'monitoring' && product.status !== 'paused' && (
